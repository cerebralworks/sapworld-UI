		...
		response => {
			...
			if(this.userInfo){
				...
				if(tempFilterdata.length==1){
					var tempCheck = this.currentJobDetails['job_locations'].filter(function(a,b){ return a.country.toLocaleLowerCase() === tempFilterdata[0]['nicename'].toLocaleLowerCase() });
	...
	**/	
	...
	onApplyJob = () => {
		if (this.resumeSelected && this.resumeSelected.file) {
			...
			requestParams.location_id = this.currentJobDetails['job_locations'][0]['id'];
